...
const Tabs: FC<TabsProps> & { Tab: FC<TabProps> } = ({children, defaultIndex = 0}) => {
  ...
  return (
    <div>
      <div className="flex">
        {tabs.map((tab, idx) => (
          <button key={idx} onClick={() => setActiveIndex(idx)} className={`flex gap-1 rounded-t-lg px-4 py-2
          ${activeIndex === idx ? `text-white bg-${tab.props.theme}` : "text-neutral-300 font-light"}`}>
            <img src={`/images/icons/${tab.props.icon.concat(activeIndex === idx ? "-white": "")}.svg`} alt='tab icon' />
            {tab.props.title}
          </button>
        ))}
      </div>
